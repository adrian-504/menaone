...
- The thread strip sits under the header of opportunity, proposal, agreement and project pages (`threadStrip.ts`, `tabs/recordThread.ts`). Every step keeps its name (Opportunity, Proposal, Agreement, Project) — it is the last thing to go. The open record shows only its step name, highlighted and not a link (the header already has its title); other steps show name and label as a link, the label cut at about 32 characters with the full text on hover; missing steps show only their muted name, the first one carrying the next action (a small ghost button). Gaps read "11d · with client", amber only when late; steps dated out of order show the line with no number. The strip wraps to a second line rather than cutting labels short, and never scrolls sideways. On the proposal page it sits quietly on the stage rail, without its own panel. Nothing shows for a lone record with nothing next. Tab reaches every link and the button.
- One timeline per record (`timeline.ts`): what happened above a red "Now" line, what's coming below it, overdue first and in red, undated under "No date". The last 15 past rows, with "Show earlier"; the future is never cut. At most one inline action per row (complete a task, mark a promise kept). Opportunity and project pages have "This opportunity / Whole engagement" (remembered, `app_meta.timeline_scope`) and "+ Log note" in its header; the timeline replaces their Activity section and sits near the top — under Next action on the opportunity, first on the project. Proposal and agreement pages use it in place of Activity (the proposal keeps its stage rail and its Notes box, whose notes stay out of the timeline). Company and contact pages keep their activity feeds; My Day shares the Now line.
...
- Company 360 is a briefing, not a count of what exists. Under the header, up to five plain clauses say where we stand (`lib/companyBrief.ts`), each only when it has something to say: the relationship and commercials (client since, services, monthly value, agreement end and its notice date); what's in flight, one line per engagement with its waiting state (three, then "and N more"); contact rhythm (last meeting, last email, next meeting — amber when an active client has had neither for over 45 days); who owes what (with overdue tasks); and pinned notes, verbatim. Every name and number in them is a link. Then Open threads (one strip per live engagement, labelled, five then "Show all"), People (decision makers first, then by last contact), the timeline, Company notes, and "All records" — the full sections, collapsed under one heading with their counts (open or closed is remembered). The section bar jumps to any of them and opens it. Key facts live in the edit panel under the header (Edit); industry, owner, location and website are in the header.
- The Brief (header "Brief", or ⌘K "Brief <company>") is one read-only page of the same facts — clauses, open threads, people, open commitments both ways, the next meeting — that prints on its own in Light.
