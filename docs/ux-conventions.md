...
- Company 360 is a briefing, not a count of what exists. Under the header, up to five plain clauses say where we stand (`lib/companyBrief.ts`), each only when it has something to say: the relationship and commercials (client since, services, monthly value, agreement end and its notice date); what's in flight — the count, naming only what is with us or late (the meeting page's brief spells each one out); contact rhythm (last meeting, last email, next meeting — amber when an active client has had neither for over 45 days); who owes what (with overdue tasks); and pinned notes, verbatim. Every name and number in them is a link. Then Open threads — one line per live engagement: its label, where it stands, who it's waiting on (amber when late) and its next step on hover; the row opens the record, where the full strip is. Five, then "Show all". Engagements with nothing dated for 120 days are dormant: one line, "N dormant — review in Clean-up". Then People (decision makers first, then by last contact; with nobody yet, one quiet line at the bottom), the timeline, Company notes, and then every record section that has something in it, always open, in a fixed order: Proposals, Agreements, Opportunities, Projects, Meetings, Tasks, Commitments, Linked notes, Files, Linked emails (owner, 23-Sep-2026: anything with content is seen, never folded away). Empty ones are the one quiet line with "+ New" after them; Linked emails, with no "+ New", is simply absent when there are none. The section bar is Overview, People, Timeline, Notes, then each populated record section with its count; it scrolls sideways without a visible bar when long, and keeps the current section in view. Key facts live in the edit panel under the header (Edit); industry, owner, location and website are in the header.
...
- Tasks → Promises (rail entry after Someday, count = open both ways; ⌘K "What do I owe" / "Owed to me"): every open commitment across clients in two sections, You owe and Owed to you, each late first, then by due date, undated last; rows are the shared row without the direction words (the section says it). An empty section is one quiet line after the other. Kept or dropped in the last 30 days fold at the bottom. On something owed to us, "…" adds Mark kept and Nudge (a `mailto:` draft to whoever promised it, "Following up: …"; never sent). No quick-add, board or calendar there.
