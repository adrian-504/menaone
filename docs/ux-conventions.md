...
- The thread strip sits under the header of opportunity, proposal, agreement and project pages (`threadStrip.ts`, `tabs/recordThread.ts`): the open record highlighted and not a link, the others links, gaps as "11d · with client" (amber only when late), and the one next action as a small ghost button. Nothing shows for a lone record with nothing next. It measures its own width: the step names go first, then "with client"; the dots, names and days stay. Tab reaches every link and the button.
- One timeline per record (`timeline.ts`): what happened above a red "Now" line, what's coming below it, overdue first and in red, undated under "No date". The last 15 past rows, with "Show earlier"; the future is never cut. At most one inline action per row (complete a task, mark a promise kept). Opportunity and project pages have "This opportunity / Whole engagement" (remembered, `app_meta.timeline_scope`) and "+ Log note" in its header; the timeline replaces their Activity section. Proposal and agreement pages use it in place of Activity (the proposal keeps its stage rail and its Notes box, whose notes stay out of the timeline). Company and contact pages keep their activity feeds; My Day shares the Now line.
- No empty boxes (`sectionLayout.ts`): on opportunity, project, meeting and company pages a section with nothing in it collapses to its heading, "None yet" and its "+ New", and sinks below the sections with content (which keep their order); clicking "None yet" opens it. The opportunity page is one column: Description and Next action first (the next open task as a link, or the text box when there is none), the timeline last. It has no Proposal or Project sections — the strip links them and offers to create them.
