...
function renderSidebar(): void {
  ...
  const smart = SMART.map((s) => item(s.key, s.label, `<span class="ws-side-icon" style="color:${s.tint}">${icon(s.icon, 15)}</span>`, s.key === 'completed' ? '' : s.key === 'today' ? dueToday : count(s.key),
